...
def agent_action(state):
    ...
    #TODO: implement action choice logic based on state
    return 0 
...
pygame.time.set_timer(AGENTEVENT,100)
...
while True:
	...
	for event in pygame.event.get():
        ...
        if event.type == AGENTEVENT:
            state = None
            #TODO: prepare state tuple from heght relative to next pipe, vertical velocity, 
            #   and x distance to next pipe
            if(agent_action(state)):
                bird_movement = -12
				flap_sound.play()
